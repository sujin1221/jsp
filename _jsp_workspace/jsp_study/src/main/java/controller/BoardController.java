...
//				destPage = "list"; //내부 list case로 이동
		...
		case "remove":
			try {
				int bno = Integer.parseInt(request.getParameter("bno")); //게시글 번호를 정수로 변환
				log.info("remove check 1"); //확인				
				//댓글, 파일도 같이 삭제
				boardVO bvo = new boardVO();
				...
				isOk = bsv.remove(bno); //게시글 삭제
				log.info("remove >>> {} ",isOk>0 ? "ok":"fail"); //확인				
				destPage = "list"; //목적 페이지 = list
			} catch (Exception e) {
