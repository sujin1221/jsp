...
protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
					...
					case "image_file":
						...
						if(item.getSize() > 0) { //데이터의 크기를 바이트 단위로 리턴 크기가 0보다 큰지 체크
							String fileName = item.getName()
									.substring(item.getName().lastIndexOf("FileSeparator")+1); //경로를 포함한 이름 ~~~/ dog.jpg
		...
		case "edit" :
			try {
				...
				for(FileItem item : itemList) {
					...
					case "new_file":
						...
						//기존 파일은 있지만, 새로운 이미지파일이 없다면...
							bvo.setImageFile(old_file);// 기존객체를 bvo에 담기
					}
						break;
				...
				destPage="list"; //해당케이스를 다시 돌아요
						
				
				
				
				
				//파라미터로 받은 bno, titile, content 데이터를
...
//			break;
		...
		rdp.forward(request, response); //요청에 필요한 객체를 가지고 destPage 경로로 이동
